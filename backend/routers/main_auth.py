...
from config import auth_service, dependencies

router = APIRouter()

@router.post("/register", response_model=user_schemas.UserOut)
def register(user: user_schemas.UserCreate, db: Session = Depends(dependencies.get_db)):
...
def login(user: user_schemas.UserCreate, db: Session = Depends(dependencies.get_db)):
    auth_user = crud_user.authenticate_user(db, user.username, user.password)
    if not auth_user:
        raise HTTPException(status_code=401, detail="Incorrect username or password")
    token = auth_service.auth_service.create_access_token({"sub": user.username})
